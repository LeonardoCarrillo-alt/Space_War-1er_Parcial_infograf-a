...
class GameView(arcade.View):
    def __init__(self):
        ...
        self.playSGame = arcade.play_sound(self.sound, volume=0.2)
    ...
    def on_update(self, delta_time):
        ...
        if len(self.enemies) == 0:
            ...
            arcade.stop_sound(self.playSGame)
            self.window.show_view(level2)

        elapsed_time = time.time() - self.start_time
        self.time_remaining = max(0, LEVEL_TIME - elapsed_time)

        if self.time_remaining <= 0:
            self.game_over = True
            game_over_view = gameOverScreen.GameOverView()
            arcade.stop_sound(self.playSGame)
    ...
    def on_draw(self):
        ...
        arcade.draw_text(f"Time: {minutes:02d}:{seconds:02d}",WIDTH - 200,HEIGHT - 40,arcade.color.WHITE,font_size=25)
